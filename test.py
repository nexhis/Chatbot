...
bot_name = "\nChatbot:"
user_name = "User:"
background = "User is talking to Chatbot, who is helpful, creative, and very friendly and knows everything. Chatbot is created by Group3."
...
def chatgpt_clone(input, history):
    ...
    s = list(sum(log_list, ()))
    chat_log = background + (' '.join(s))
    answer = ask(input, chat_log)

    user_log = f"{user_name} {input}"
    bot_log = f"{bot_name} {answer}"

    log_list.append((user_log, bot_log))   # Add the current input and response to the chat history
